...
We now have some tools for creating quantitative data as well as categorical.  Which means we can make a dataset!  Maybe this is for a simulation or maybe it's a simple example for testing complicated code.  We'll do several simple ones to get the idea.

Let's start by simulating data that we would use in a simple two-sample analysis with no difference between groups.

We need to use the tools we learned above but we now name results and put them into a data.frame (this latter step isn't always necessary, but does help keep things organized in certain types of simulations).

One option is to make separate vectors of different lengths and then bind them together via `data.frame()`.  Notice there is no need to use `cbind()` here, which we often think we need as R beginners (I know I did!).  Instead we can use `data.frame()` directly.
...
The dataset so far has been one with "no difference" among groups.  What if the means were different between groups.  Let's make two groups of three observations where the mean of one group is 5 and the other 10 but with a shared variance of 1.

Remembering how `rnorm()` works with a vector of means is key here.  The function draws iteratively from each distribution.  To get the groups in the correct order we need to use `times`.
...
For our last dataset let's make two unrelated variables, one between 10 and 15 and one between 100 and 150.  We can add some groups here, so let's have 10 obserations of each quantitative variable in each group.

Sometimes it's keeping track of the different numbers that can be hard.  We use `each = 10)` to repeat each element of the vector 10 times to get a total of 20 but use n = 20 in `runif()`.
...
The `replicate()` function is a real workhorse when making repeated simulations.  It is a member of the *apply* family in R, and is specifically made (per the documentation) for the *repeated evaluation of an expression (which usually involves random number generations)*.  We want to repeatedly simulate data that invovles random number generation, so that sounds like a useful tool!

The `replicate()` function takes three arguments:

* `n`, which is the number of replications to perform.  This is to set the number of repeated runs desired.  
...
Let's say we want to simulate some values from a normal distribution, which we can do using the `rnorm()` function as above.  But now instead of drawing some number of values from a distribution one time I want to do it many times.  This could be a classic example of the central limit theorem or something.
...
```{r}
set.seed(16)
replicate(n = 3, expr = rnorm(5, 0, 1), simplify = FALSE )
```
...
# Now that we have the tools, what's next?

For a simulation to explore long-run behavior there is going to be some sort of repeating the process, which we did via `replicate()`, and then getting whatever results are of interest.  This latter process is often going to involve some sort of looping.  By saving our generated variables or data.fraes into a list we've made it so we can loop via list looping functions like `lapply()` or `purrr::map()`.  The family of *map* functions are newer and have a lot of convenient output types that make them pretty useful.  I go over a few of those in my blog post [A closer look at replicate() and purrr::map() for simulations](https://aosmith.rbind.io/2018/06/05/a-closer-look-at-replicate-and-purrr/).
