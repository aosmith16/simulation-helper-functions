...
We'll use the data in a regression model fit via `lm()`.  Take note of the slope.

```{r}
lm(y ~ x1)
```

Now simulate a new dataset with the same `y` but an explanatory variable with values between 100 and 200.  Since `x2` has values 100x larger than `x`, the new slope coefficient is much smaller in absolute value.

```{r}
x2 = runif(n = 100, min = 100, max = 200)
head(x2)
```
