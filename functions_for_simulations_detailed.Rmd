...
Now simulate a new dataset with the same `y` but an explanatory variable with values between 100 and 200.  Since I set the seed you'll see new slope coefficient is exactly 1/100th of the coefficient above, `-0.0007983`.
...
We can use `times` with a vector instead of an integer value to repeat each element of the character vector a different number of times.  This is one way to simulate unbalanced groups.  Unlike our first use of `times`, using `times` with a vector repeats each element like `each` does.
...
We need to repeat the values in a way that every combination of `group` and `factor` is present in the dataset one time.
...
This is a job for the `times` argument, so the whole vector repeats two times.  In the result below we can see every combination of the two factors is present once.
...
We need to repeat the whole vector twice instead of elementwise.
...
But if we get things wrong and the number we use goes into the number we need evenly, R will *recycle* the vector to the end of the `data.frame()`.  This is a hard mistake to catch.
