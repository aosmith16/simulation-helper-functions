...
3. Learn to use `replicate()` to repeat the data simulation process many times

# Generating random numbers

An easy way to generate numeric data is to pull random numbers from some distribution.  This can be done via the functions for pulling random deviates from some distribution.  These functions always start with `r` (for "random").
...
If we set the seed, we can reproduce the values we generated.
...
If we run the code again, we get the same values.
...
For getting a quick set of numbers it's easy to use the default parameter values in `rnorm()`.  When I'm exploring long-run behavior of variance estimated from linear models, though, I will explore different standard deviation values.
...
I've started using `runif()` pretty regularly, especially when I want to generate numbers that are strictly positive but continuous.  The uniform distribution is a continuous distribtuion, with numbers uniformly distributed between some minimum and maximum.
...
The dataset above is one with "no difference" among groups.  What if the means were different between groups?  Let's make two groups of three observations where the mean of one group is 5 and the other 10.  The two groups have a shared variance (and so stanard deviation) of 1.
...
I've found that getting the numbers correct when working with simulations with both quantitative and categorical variables takes some practice.  
