...
The `n` argument is the number of observations we want to generate.  The `mean` and `sd` arguments show what the default values are (note that `sd` is the *standard deviation*, not the variance).  Not all `r` functions have defaults like this.
...
Making sure someone else will be able to exactly reproduce your results when running the same code can be desirable for teaching and is useful when making an example dataset for demonstarting a coding issue.

I also will set the seed when I'm making a function for simulations and I want to make sure it works correctly.  Otherwise in most simulations we don't actually want to set the seed.
...
I've seen others change the mean and standard deviation to create a variable that is within some specific range.
...
```
 ...
 If a vector is passed to `n`, however, then the length of that vector is taken to be the number required (see `Arguments` section of documentation).
...
One situation where I've found `runif()` handy is when I wanted to demonstrate the effect of the relative size of the variable values on the size of the estimated coefficient in a regression.  For example, the relative size of a variable measured  in kilometers is smaller than if that variable was converted into meters.

Let's generate some data with the response variable (`y`) pulled from a standard normal distribution and the explanatory variable with values between 1 and 2.  The two variables are unrelated.  You see I'm still writing out my argument names for clarity, but you may get a sense how easy it would be to start cutting corners to avoid the extra typing.

Let's set the seed so we can reproduce the explanatory variable with a different order of magnitude.

```{r}
set.seed(16)
x1 = runif(n = 10, min = 1, max = 2)
y = rnorm(n = 10, mean = 0, sd = 1)
x1
```

We'll use the data in a regression model fit via `lm()`.  Take note of the slope, estimated to be about ``-0.07983`.

```{r}
lm(y ~ x1)
```

Now simulate a new dataset with the same `y` but an explanatory variable with values between 100 and 200.  You'll see new slope coefficient is exactly 1/100th of the coefficient above, `-0.0007983`.

```{r}
set.seed(16)
x2 = runif(n = 10, min = 100, max = 200)
x2
```
...
I use `rpois()` for generating counts for generalized linear models.  I've also found this function useful in gaining a better understanding of the shape of the Poisson distribution with different means.
...
Let's explore the Poisson distribution a little more, seeing how the distribution looks when changing the mean.  Being able to look at how the Poisson distribution changes with the mean via simulation helped me understand the the distribution better, and helpd me become a lot more aware of why it so often does a poor job for ecological count data.
...
The first argument of `rep()` is the vector to be repeated.  One option is to write out the character vector you want to repeat.  You can also get a simple character vector through the use of `letters` or `LETTERS`.  These are *built in constants* in R.  `letters` is for the 26 lowercase letters of the Roman alphabet and `LETTERS` is for 26 uppercase letters.

Letters can be pulled out via the extract brackets (`[`).  I use these built-in constants for pure convenience when I need to make a basic categorical vector.  I find it more straightforward to type out the word and brackets than the vector of characters (complete with all those pesky quotes and such).
...
The `times` argument can be used when we want to repeat the whole vector rather than repeat it elementwise like we did above with `each`. 
...
## Using the `length.out` argument to define the vector length
...
We now have some tools for creating quantitative data as well as categorical.  Which means it's time to make some datasets!  We'll create several simple ones to get the idea.
...
Say we are in a situation involving two factors, not one.  We have two observations for every combination of the two factors (i.e., the two factors are *crossed).    

The second factor, which we'll call `factor`, will take on the values "C", "D", and "E".  We need to repeat the variable with a specific pattern so every combination of `group` and `factor` is present in the dataset and has two observations.

What argument do we use in `rep()`?

```{r, eval = FALSE}
factor = rep(LETTERS[3:5], ? = 2)
```

This is a job for the `times` argument, to repeat the whole vector two times.
...
The dataset above is one with "no difference" among groups.  What if the means were different between groups?  Let's make two groups of three observations where the mean of one group is 5 and the other 10.  The two groups have a shared variance (and so standard deviation) of 1.
...
For our last dataset let's make two unrelated quantitative variables, one between 10 and 15 and one between 100 and 150. We also have two groups, with 10 observations in each group.
...
Here we use `each = 10` to repeat each element of the vector 10 times to get a total of 20 observations.  We are drawing values of the quantitative values overall, though, not by group, so we need to use `n = 20` in `runif()`.
