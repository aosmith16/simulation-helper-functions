...
I like `runif()` to demonstrate how the relative size of the explanatory variable affects an estimated coefficient.

First, generate an explanatory variable between 1 and 2.  Set the seed to reproduce the `x` with a different order of magnitude.  
...
The Poisson distribution is a single parameter distribution, which is called `lambda` in `rpois()`.
...
Getting the number of repeats of each categorical value correct to match the number of draws needed to make a quantitative variable of the same length is key.

How many random draws do we need for the quantitative variable?
...
The `replicate()` function is a real workhorse when making repeated simulations as it is for *repeated evaluation of an expression (which usually involves random number generations)*.
