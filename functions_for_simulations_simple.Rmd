...
Let's pull from distributions with different means but the same standard deviation.
...
First, generate an explanatory variable between 1 and 2.    
...
Then generate an explanatory variable between 100 and 200.  The new slope coefficient is much smaller. 
...
The `replicate()` function is a real workhorse when making repeated simulations as it is for *repeated evaluation of an expression (which will usually involves random number generation)*.
...
Tou can see a few examples of `replicate()` followed by `map()` in my blog post [A closer look at replicate() and purrr::map() for simulations](https://aosmith.rbind.io/2018/06/05/a-closer-look-at-replicate-and-purrr/).
