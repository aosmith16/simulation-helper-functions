...
One way to generate numeric data in R is to pull random numbers from some distribution.  This can be done via the functions for generating random deviates.  These functions start with `r` (for *random*).
...
I use the `rnorm()` function a lot, sometimes with good reason and other times when I need some numbers and I don't care too much about what they are.

There are three arguments to `rnorm()`.  From the **Usage** section of the documentation:

> rnorm(n, mean = 0, sd = 1)

- The `n` argument is the number of observations to generate.  
- The `mean` and `sd` arguments show what the default values are set to (note that `sd` is the *standard deviation*, not the variance).  
...
To write clearer code I can include the argument names and define values for the mean and standard deviation even though I'm using the default values.  
...
You can define both the mean and standard deviation to create a variable that is within some specific range.  For example, if the mean is large and the standard deviation small in relation to the mean you can generate strictly positive numbers.  (*I usually use `runif()` for this, which you'll see later in the tutorial.*)
...
You can pull random numbers from different normal distributions with distinct parameters if you use vectors instead of a single scalar value for the `mean` and/or `sd`.  This would be useful for simulating data with different group means but the same variance, for example.  You could use this approach if you were creating data to analyze using an ANOVA.
...
The `rnorm()` function work differently when using a vector for the `n` argument, though, which is a little confusing.  If a vector is passed to `n`, the *length* of that vector is taken to be the sample size required (see the **Arguments** section of documentation for details).
...
If you comment out the `set.seed()` code in the code box you can run the code block multiple times and see how the regression coefficients change.
...
I use `rpois()` for generating counts when exploring generalized linear models, for example.  I've also found this function useful in gaining a better understanding of the shape of Poisson distributions with different means.
...
Generate 5 values from a Poisson distribution with a mean of 2.5.  Note that *mean* of the Poisson distribution can be any non-negative value even though the observed values will be discrete integers (i.e., the mean does not have to be an integer).
...
Let's explore the Poisson distribution a little more to see how the distribution looks as the mean changes.  This simulation task is what helped me understand the distribution better, including my understanding of why it so often does a poor job modeling ecological count data.
...
Make note of the range of the values you get.  Run the code several times to see more examples of the range when the mean is 5.  The largest value I saw after several runs of the code was 16 and the smallest was 0 or 1.
...
Letters can be pulled out via the extract brackets (`[`).  I use these built-in constants for pure convenience when I need to make a basic categorical vector and it doesn't matter what form those categories take.  I find it more straightforward to type out the word and brackets than a complex vector of characters (complete with all those pesky quotes and such `r emo::ji("stuck_out_tongue_winking_eye")`).
...
The resulting vector is 6 observations long.  

```{r each, exercise = TRUE}
rep(x = letters[1:2], each = 3)
```

In the code above you see I wrote out the first argument name, `x`. You will most often see me skip this when using `rep()` since the argument name isn't particular useful for remembering what that argument does.
...
Like `times`, the `length.out` argument repeats the whole character vector.  However, it repeats the vector only until the length you've defined in the argument is reached.  Using `length.out` is one way to get unbalanced groups.  
...
Repeat the two letters twice each but now with a total final vector length of 7.
...
Note you cannot use `length.out` and `times` together.  If you try to do this, `length.out` will be given priority and `times` will be (silently) ignored.

## Simulating datasets

You now have tools for creating quantitative data as well as categorical data.  Which means it's time to make some datasets.  
...
I'll walk through the creation of several simple datasets so you have a chance to practice the general idea.

###

### No differences among two groups

You'll start by simulating data that you would use in a simple two-sample analysis where there is no differences between the two groups.  You'll make a total of 6 observations, three per group.
...
I wrote in the code to combine the two variables you make into an (unnamed) data frame, since you may not have seen this before.
...
Say you are in a situation involving two factors instead of one.  You have a single observations for every combination of the two factors (i.e., the two factors are *crossed*).    
...
The dataset you made above is one with no difference among the two groups.  What if you want to create data where the means are *different* between groups?  
...
You will also create two quantitative variables, unrelated to both each other and the groups.  One variable will from 10 and 15 and the other from 100 and 150. 
...
The following code example generates 5 values from a standard normal distribution three times.  Notice the addition of `simplify = FALSE` to get the output as a list.  I set the seed to get the same values here as in some later exercises.
...
A `for()` loop can be used in place of `replicate()` for simulations.  With time and practice I've personally found `replicate()` to be more convenient in terms of writing the code.  However, in my experience some folks find `for()` loops intuitive when they are starting out in R so wanted to give an example.
